...
// Maps an image-space middle/right-button drag onto a LineRequest. The image
...
// (middle-button) drag produces a line along the first in-plane axis with the
// cursor's coordinate on the second in-plane axis fixed; a vertical
// (right-button) drag swaps the roles. In 3-D the normal coordinate is fixed
